...
import {
  ...
  unassignEmployeeFromTable,
} from "../../store/employee-tables-actions.js";
...
export default function EmployeesInATable({ idOfTable }) {
  ...
  useEffect(() => {
 dispatch(getAssignEmployeeAndTableDetails(idOfTable)); 
  }, [dispatch, idOfTable]);
  const newUpdatedTabDetails =assignEmployeeAndTableDetails.filter(value=>value.tableId === idOfTable);

  function handleDeleteEmployeeFromTable(id) {    
    dispatch(deleteEmployeeFromTableDetail(id));
  }

  return (
    <>
      {newUpdatedTabDetails?.map((empAndTabDetail) => (
        <li key={empAndTabDetail["employeeTableId"]}>
          <div className="inline-flex items-center whitespace-nowrap bg-gray-100 hover:bg-gray-200 rounded-2xl p-1 mb-1 ">
            <span className="sm:me-1">{empAndTabDetail.name}</span>
            <Button
              textOnly={true}
