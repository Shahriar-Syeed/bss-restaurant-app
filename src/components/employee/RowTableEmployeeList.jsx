...
export default function RowTableEmployeeList({ deleteEmployee }) {
  ...
  const employeeList= useSelector(state=>  state.employees.employeeDataTable.data);
    ...
    <>
      ...
      {employeeList?.map((employee) => (
        <tr
          ...
          key={employee.id}
        >
        {modalId === employee.id && <Modal open={isOpen} onClose={closeModal} >
        <h1 className="md:text-xl mb-2">{`Do you want to delete employee ${employee.user.fullName ? employee.user.fullName :"of your restaurant"}!`}</h1>
          ...
          <td className="block sm:table-cell md:px-2 xl:px-4 xl:py-3 lg:px-3 lg:py-2 p-1 break-words md:break-normal" data-th='Email: '>
            {employee.user.email}
          </td>
          <td className="block sm:table-cell md:px-2 xl:px-4 xl:py-3 lg:px-3 lg:py-2 p-1 break-words md:break-normal" data-th='Phone: '>
            {employee.user.phoneNumber}
          </td>
          <td className="block sm:table-cell md:px-2 xl:px-4 xl:py-3 lg:px-3 lg:py-2 p-1 break-words md:break-normal" data-th='Join Date: '>
            {employee.joinDate}
          </td>
